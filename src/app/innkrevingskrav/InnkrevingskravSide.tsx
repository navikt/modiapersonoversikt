                ...
                <Table.Body>
                  {data.posteringer.map((p) => (
                    <Table.Row key={p.kode}>
                      ...
                      <Table.DataCell>
                        {formaterNOK(p.opprinneligBelop)}
                      </Table.DataCell>
                      <Table.DataCell>
                        {formaterNOK(p.gjenstaendeBelop)}
                      </Table.DataCell>
                    </Table.Row>
                  ))}
                </Table.Body>
    ...
    <Box>
      {debitor && (
        <VStack marginBlock="2">
          ...
          <InfoPoint
            label={debitorType === "FNR" ? "Fnr" : "Org. nr"}
            text={debitor.ident}
          />
        </VStack>
      )}
  ...
  return (
    ...
    >
      {data ? (
        ...
        <Detail>Fant ingen innkrevingskrav</Detail>
      )}
    </QueryErrorBoundary>
  );
};
  ...
  return (
    ...
    >
      {data ? (
        ...
        <Detail>Fant ikke innkrevingskravet</Detail>
      )}
    </QueryErrorBoundary>
  );
};
...
const InnkrevingskravSide = () => {
  ...
  const [searchType, setSearchType] = useState<"kravId" | "fnr" | "orgnr">(
    "kravId",
  );
  ...
  const setKravId = (clear?: boolean) =>
    navigate({ search: { kravId: clear ? undefined : nyKravId } });
      ...
      <HStack gap="6" minHeight="90vh">
          ...
          <Box marginInline="4">
            ...
            >
              ...
              >
                ...
                <IfFeatureToggleOn
                  toggleID={FeatureToggles.FnrSokForInnkreving}
                >
                ...
                <IfFeatureToggleOn
                  toggleID={FeatureToggles.OrgnrSokForInnkreving}
                >
            ...
            {searchType !== "kravId" && nyIdent && (
              <SearchResults
                id={nyIdent}
                identType={searchType === "fnr" ? "FNR" : "ORG_NR"}
              />
            )}
            {searchType === "kravId" && kravId && (
              <KravSearchResults kravId={kravId} />
            )}
          </Box>
        </Sidebar>
        <Box padding="4" flexGrow="1">
          ...
          {!kravId ? (
            <Box marginBlock="4">
              <Alert variant="info">
                Gjør et søk i sidemenyen for å velge et krav
              </Alert>
            </Box>
          ) : (
