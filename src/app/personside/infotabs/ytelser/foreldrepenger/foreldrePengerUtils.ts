...
function getAlleKommendeUtbetalinger(foreldrePenger: Foreldrepengerettighet) {
    return foreldrePenger.periode.reduce(
        (acc: KommendeUtbetaling[], periode: Foreldrepengerperiode) => [...acc, ...periode.kommendeUtbetalinger],
        []
    );
}
...
export function utledMaksDato(foreldrePenger: Foreldrepengerettighet): string {
    ...
    try {
        ...
        loggError(error, 'Mangler data for å utlede maks dato', {
            alleKommendeUtbetalinger: alleKommendeUtbetalinger,
        });
        return 'Mangler data';
    }
}
